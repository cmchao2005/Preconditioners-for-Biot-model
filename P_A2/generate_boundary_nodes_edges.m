...
%Modify the top boundary nodes to be Neumann.
for k=2*N1_basis+2:3*N1_basis
    boundary_nodes(1,k)=-2;
end
...
for k=1:N1_basis
    boundary_nodes(2,k)=k;
end

%right boundary nodes.
for k=N1_basis+1:N1_basis+N2_basis
    boundary_nodes(2,k)=(k-N1_basis-1)*(N2_basis+1)+N1_basis+1;
end

%top boundary nodes.
for k=N1_basis+N2_basis+1:2*N1_basis+N2_basis
    boundary_nodes(2,k)=(N1_basis+1)*(N2_basis+1)-k+N1_basis+N2_basis+1;
end

%left boundary nodes.
for k=2*N1_basis+N2_basis+1:nbn
    boundary_nodes(2,k)=(N1_basis+1)*(N2_basis)+1-(k-2*N1_basis-N2_basis-1)*(N1_basis+1);
end
...
for k=1:N1_partition
    ...
    boundary_edges(6,k)=-1;  %normal vector
end

%Modify the top boundary edges to be Neumann.
for k=N1_partition+N2_partition+1:2*N1_partition+N2_partition
    boundary_edges(1,k)=-2;
    boundary_edges(6,k)=1;
end

%The index in the following is associated with the index in "generate_M_T_triangle.m".
%bottom boundary edges.
for k=1:N1_partition
    boundary_edges(2,k)=2*k-1;
    boundary_edges(3,k)=k;
    boundary_edges(4,k)=k+1;
end

%right boundary edges.
for k=N1_partition+1:N1_partition+N2_partition
    boundary_edges(2,k)=(k-N1_partition)*2*N1_partition;
    boundary_edges(3,k)=(k-N1_partition)*(N1_partition+1);
    boundary_edges(4,k)=(k-N1_partition+1)*(N1_partition+1);
end

%top boundary edges.
for k=N1_partition+N2_partition+1:2*N1_partition+N2_partition
    boundary_edges(2,k)=2*N1_partition*N2_partition-2*(k-N1_partition-N2_partition-1);
    boundary_edges(3,k)=(N1_partition+1)*(N2_partition+1)-(k-N1_partition-N2_partition-1);
    boundary_edges(4,k)=(N1_partition+1)*(N2_partition+1)-(k-N1_partition-N2_partition);
end

%left boundary edges.
for k=2*N1_partition+N2_partition+1:nbe
    boundary_edges(2,k)=2*N1_partition*N2_partition-(k-2*N1_partition-N2_partition)*2*N1_partition+1;
    boundary_edges(3,k)=(N1_partition+1)*N2_partition+1-(k-2*N1_partition-N2_partition-1)*(N1_partition+1);
    boundary_edges(4,k)=(N1_partition+1)*N2_partition+1-(k-2*N1_partition-N2_partition)*(N1_partition+1);
end
