function b=treat_Neumann_boundary_triangle(Neumann_boundary_function_name,b,boundary_edges,M_partition,T_partition,T_basis_test,number_of_test_local_basis,current_time,test_basis_type,test_derivative_degree_x,test_derivative_degree_y)
...
%If the kth boundary edge is a Neumann boundary edge,then we add the corresponding line integral to b.
    ...
    if boundary_edges(1,k)==-2 
        ...
        for beta=1:number_of_test_local_basis
            temp=Gauss_quadrature_for_line_integral_test_triangle(current_time,Neumann_boundary_function_name,Gauss_coefficient_reference_1D,Gauss_point_reference_1D,end_point_1,end_point_2,vertices,test_basis_type,beta,test_derivative_degree_x,test_derivative_degree_y);
            b(T_basis_test(beta,element_index),1)=b(T_basis_test(beta,element_index),1)+boundary_edges(6,k)*temp;
        end
